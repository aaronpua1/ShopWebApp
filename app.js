...
app.post('/create-offer', function(req, res) {
    ...
    console.log("upsell selections:" + upsell_selections[0].id);
    console.log("product selections:" + product_selections[0].id);
    ...
    console.log("upsell diff:" + upsell_differences[0].handle);
    console.log("product diff:" + product_differences[0].id);
    async.parallel([
        function(callback) {
    ...
    async.waterfall([
        ...
        function(values, callback) {
            ...
            for (var i in values) {
                //values[i] = JSON.parse(values[i]);
                for (var j in values[i].metafields) {
                    var temp = JSON.parse(JSON.stringify(parse_values(values[i].metafields[j].value)));
                    var data = {
                        metafield: {
                            id: values[i].metafields[j].id,
                            value: "handle:" + temp.handle + ";status:on",
                            value_type: "string"
                        }
                    }
                    req_body = JSON.stringify(data);
